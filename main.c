...
void render_battle_scene(SDL_Renderer* renderer, Character* player, Enemy* current_monster, int difficulty, int is_boss, int menu_index, int hide_monster)
{
	...
	char stat_p[32];
	sprintf(stat_p, "LV:%d", player->level);
...
{	
	...
	while(!quit)
	{
				...
				else if (game_state == 99 && !player.is_moving)
				{
					...
					else if (KEY == SDLK_z || KEY == SDLK_RETURN) 
					{
						...
						if (chk_x >= 0 && chk_x < MAP_WIDTH && chk_y >= 0 && chk_y < MAP_HEIGHT)
						{
							...
							{
								// 무기 공격력이 0일 때(층에 처음 왔을 때)만 새로 굴리기! (꼼수 차단)
								if (g_current_shop_weapon.atk_bonus == 0) 
								{
									...
									// ★ 여기서 수치 조절해! 레어는 +10, 에픽은 +30으로 확 올려써어! ★
									int base_atk = (difficulty == 0) ? 10 : ((difficulty == 1) ? 20 : 30);
									int extra_atk = (g_current_shop_weapon.rarity == RARITY_EPIC) ? 30 : ((g_current_shop_weapon.rarity == RARITY_RARE) ? 10 : 0);
				...
				for (int cx = 0; cx < 17; cx++)
				{
					...
					if (mx >= 0 && mx < MAP_WIDTH && my >= 0 && my < MAP_HEIGHT)
					{
						...
						else
						{
							...
							else if (t == TILE_BOSS_CHEST) { SDL_Rect bch = {48, 64, 16, 16}; SDL_RenderCopy(renderer, map_tex, &bch, &d_r); }
							else if (t == TILE_NPC) { SDL_Rect npc = {112, 0, 16, 16}; SDL_RenderCopy(renderer, char_tex, &npc, &d_r); }
							else if (t == TILE_TORCH) { SDL_Rect to = {64, 64, 16, 16}; SDL_RenderCopy(renderer, map_tex, &to, &d_r); }
		...
		{
			// [UI 수정 위치: 세이브/로드 메뉴]
			draw_dw_window(renderer, 100, 50, 610, 500);
			const char* title = (game_state == STATE_SAVE_MENU) ? "--- SAVE DATA ---" : "--- LOAD DATA ---";
			render_text(renderer, title, 230, 80, 255, 255, 0);
			...
			for (int i = 0; i < 5; i++)
			{
				...
				else
				{
					...
					sprintf(slot_info, "[SLOT %d] LV:%d %s", i + 1, g_save_slots[i].player_data.level, c_name);
					render_text(renderer, slot_info, 150, slot_y, 255, 255, (menu_index == i ? 0 : 255));
				}
			}
			render_text(renderer, "PRESS ESC TO CANCEL", 215, 500, 200, 200, 200);
		}
		else if (game_state == STATE_ESC_MENU) 
		{
			// [UI 수정 위치: 던전 탐험 중 ESC 메뉴 박스]
			draw_dw_window(renderer, 240, 150, 400, 350);
			render_text(renderer, "--- MENU ---", 305, 180, 255, 255, 255);

			render_text(renderer, "STATUS", 295, 230, 255, 255, (menu_index == 0 ? 0 : 255));
			render_text(renderer, "EQUIPMENT", 295, 270, 255, 255, (menu_index == 1 ? 0 : 255));
			render_text(renderer, "INVENTORY", 295, 310, 255, 255, (menu_index == 2 ? 0 : 255));
			render_text(renderer, "SAVE DATA", 295, 350, 255, 255, (menu_index == 3 ? 0 : 255));
			render_text(renderer, "QUIT TO TITLE", 295, 390, 255, 255, (menu_index == 4 ? 0 : 255));
			render_text(renderer, "RETURN TO GAME", 295, 430, 255, 255, (menu_index == 5 ? 0 : 255));

			int cursor_y = 230 + (menu_index * 40);
			render_text(renderer, ">", 265, cursor_y, 255, 255, 0);
		}
		else if (game_state == STATE_STATUS_MENU) 
		{
			// [UI 수정 위치: ESC 하위 - 상태창]
			draw_dw_window(renderer, 180, 100, 490, 400);
			render_text(renderer, "-- STATUS --", 310, 130, 255, 255, 0);
			...
			render_text(renderer, "PRESS ESC TO RETURN", 210, 470, 200, 200, 200);
		}
		else if (game_state == STATE_EQUIP_MENU) 
		{
			// [UI 수정 위치: 장비창 왼쪽 - 보유 장비 리스트]
			draw_dw_window(renderer, 50, 50, 500, 500);
			...
			draw_dw_window(renderer, 570, 50, 230, 500);
			render_text(renderer, "PREVIEW", 600, 80, 255, 255, 0);
			...
			if (player.weapon_count > 0)
			{
				...
				char p_buf[64];
				sprintf(p_buf, "ATK: %d", player.atk + current_bonus);
				render_text(renderer, p_buf, 600, 180, 200, 200, 200);

				if (diff >= 0) sprintf(p_buf, "->ATK: %d (+%d)", new_total, diff);
				else sprintf(p_buf, "->ATK: %d (%d)", new_total, diff);

				int p_r = (diff > 0) ? 50 : (diff < 0 ? 255 : 255);
				int p_g = (diff > 0) ? 255 : (diff < 0 ? 50 : 255);
				int p_b = (diff > 0) ? 50 : (diff < 0 ? 50 : 255);
				render_text(renderer, p_buf, 570, 240, p_r, p_g, p_b);
			}

			render_text(renderer, "ENTER: E", 600, 440, 255, 255, 0);
			render_text(renderer, "ESC: R", 600, 480, 150, 150, 150);
		}
		else if (game_state == STATE_INVEN_MENU) 
		{
			// [UI 수정 위치: ESC 하위 - 인벤토리]
			draw_dw_window(renderer, 180, 150, 490, 300);
			render_text(renderer, "-- INVENTORY --", 290, 180, 255, 255, 0);
			char buf[64];
			...
			render_text(renderer, "PRESS ESC TO RETURN", 210, 410, 200, 200, 200);
		}
		else if (game_state == STATE_SHOP_UI)
		{
			// [UI 수정 위치: 상점창]
			draw_dw_window(renderer, 50, 150, 540, 300);
			render_text(renderer, "--- MERCHANT ---", 130, 180, 255, 255, 255);

			char price_buf[64];
			sprintf(price_buf, "BUY HP POTION (50G)"); render_text(renderer, price_buf, 110, 240, 255, 255, (menu_index == 0 ? 0 : 255));
			sprintf(price_buf, "BUY MP POTION (100G)"); render_text(renderer, price_buf, 110, 290, 255, 255, (menu_index == 1 ? 0 : 255));
			...
			render_text(renderer, price_buf, 110, 340, w_r, w_g, w_b);
			render_text(renderer, "LEAVE", 110, 390, 255, 255, (menu_index == 3 ? 0 : 255));

			int cursor_y = 240 + (menu_index * 50);
			render_text(renderer, ">", 80, cursor_y, 255, 255, 0);

			// [UI 수정 위치: 골드 표시 박스 - 여기가 수정된 부분이야!]
			draw_dw_window(renderer, 610, 150, 150, 100);
			render_text(renderer, "GOLD", 630, 170, 255, 255, 255);
			sprintf(price_buf, "%d G", player.gold); 
			render_text(renderer, price_buf, 630, 210, 255, 255, 0); // price_buf를 인자로 꼭 넣어줘써어!
		}
