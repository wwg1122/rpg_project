...
void render_battle_scene(SDL_Renderer* renderer, Character* player, Enemy* current_monster, int difficulty, int is_boss, int menu_index, int hide_monster)
{
    ...
    // [ADD] 장비 공격력을 포함하여 표시
	sprintf(stat_p, "ATK: %d(+%d)", player->atk, player->equip_weapon_atk); 
...
{	
	...
	while(!quit)
	{
					...
					for (int i = 0; i < g_monster_count; i++)
					{
						if (g_monsters[i].is_alive && g_monsters[i].x == player.x && g_monsters[i].y == player.y)
						{
							if (g_monsters[i].ignore_turns <= 0)
							{
								...
								saved_map_x = current_monster->x;
								saved_map_y = current_monster->y;
								is_boss = (g_dungeon_map[current_monster->y][current_monster->x] == TILE_BOSS);

								play_encounter_transition(renderer);
								sprintf(g_msg1, "AMBUSHED BY %s!", current_monster->name);
								g_msg2[0] = '\0';
								game_state = 3;
								menu_index = 0;
								break;
		...
		while (SDL_PollEvent(&e) != 0)
		{
			...
			else if (e.type == SDL_KEYDOWN)
			{
				if (KEY == SDLK_ESCAPE)
				{
                    ...
                    {
                        game_state = STATE_ESC_MENU; menu_index = 0;
                    }
                    ...
                    {
                        game_state = STATE_ESC_MENU; menu_index = 3; 
                    }
                    else if (game_state == STATE_LOAD_MENU)
                    {
                        game_state = 0; menu_index = 1;
                    }
                    else if (game_state >= STATE_STATUS_MENU && game_state <= STATE_INVEN_MENU)
                    {
                        game_state = STATE_ESC_MENU; 
                    }
				...
				else if (game_state == 1)
				{
					...
					else if (KEY == SDLK_RETURN)
					{
                        ...
                        player.equip_weapon_atk = 0; player.equip_armor_def = 0;
                        sprintf(player.weapon_name, "NONE"); sprintf(player.armor_name, "NONE");
                ...
                else if (game_state == STATE_SAVE_MENU || game_state == STATE_LOAD_MENU)
                {
                    ...
                    else if (KEY == SDLK_RETURN)
                    {
                        ...
                        {
                            g_save_slots[menu_index].is_empty = 0;
                            g_save_slots[menu_index].player_data = player;
                            g_save_slots[menu_index].hp_potions = hp_potions;
                            g_save_slots[menu_index].mp_potions = mp_potions;
                            g_save_slots[menu_index].difficulty = difficulty;
                            g_save_slots[menu_index].current_floor = current_floor;
                            write_save_data();
                            game_state = STATE_ESC_MENU; menu_index = 0;
                        }
                        else if (game_state == STATE_LOAD_MENU)
                        {
                            if (!g_save_slots[menu_index].is_empty)
                            {
                                ...
                                hp_potions = g_save_slots[menu_index].hp_potions;
                                mp_potions = g_save_slots[menu_index].mp_potions;
                                difficulty = g_save_slots[menu_index].difficulty;
                                
                                current_skills = (player.class_type == 0) ? warrior_skills : mage_skills;
                                current_skill_count = 2;
                                game_state = 2; menu_index = 0;
                            }
                        }
                ...
                else if (game_state == STATE_ESC_MENU)
                {
                    ...
                    else if (KEY == SDLK_RETURN)
                    {
                        ...
                        else if (menu_index == 1) { game_state = STATE_EQUIP_MENU; } 
                ...
                else if (game_state == STATE_SHOP_UI)
                {
                    ...
                    if (KEY == SDLK_UP) { if (menu_index > 0) menu_index--; }
                    else if (KEY == SDLK_DOWN) { if (menu_index < 3) menu_index++; } // 항목 4개 (무기추가)
                    else if (KEY == SDLK_RETURN)
                    {
                        ...
                        else if (menu_index == 2) // 무기 구매
                        { 
                            if (player.gold >= price_wpn) 
                            { 
                                player.gold -= price_wpn; 
                                player.equip_weapon_atk = 10;
                                if (player.class_type == 0) sprintf(player.weapon_name, "IRON SWORD");
                                else sprintf(player.weapon_name, "WOODEN WAND");
                            } 
                        }
					...
					else if (KEY == SDLK_z || KEY == SDLK_RETURN) 
					{
						...
						if (chk_x >= 0 && chk_x < MAP_WIDTH && chk_y >= 0 && chk_y < MAP_HEIGHT)
						{
							...
							if (target_tile == TILE_CHEST || target_tile == TILE_BOSS_CHEST)
							{
                                ...
                                if (target_tile == TILE_BOSS_CHEST)
                                {
                                    if (player.class_type == 0) {
                                        sprintf(msg, "GOT HERO SWORD!");
                                        player.equip_weapon_atk = 30;
                                        sprintf(player.weapon_name, "HERO SWORD");
                                    } else {
                                        sprintf(msg, "GOT SAGE WAND!");
                                        player.equip_weapon_atk = 30;
                                        sprintf(player.weapon_name, "SAGE WAND");
                                    }
                                }
                                else // 일반 포션 상자
                                {
				...
				else if (game_state == 3) // 전투 로직
				{
					...
					else if (KEY == SDLK_RETURN)
					{
						switch (menu_index)
						{
							...
							{
                                // [ADD] 무기 공격력 합산
                                int total_atk = player.atk + player.equip_weapon_atk;
								...
								if (current_monster->hp <= 0)
								{
                                    ...
                                    int gained_exp = current_monster->exp_reward;
                                    int gained_gold = current_monster->gold_reward;
                                    player.exp += gained_exp;
                                    player.gold += gained_gold;

                                    char reward_msg[64];
                                    sprintf(reward_msg, "GOT %d EXP & %d G!", gained_exp, gained_gold);
									...
									/* 보스 사망 시 상자와 포탈을 남기고 필드로 복귀 */
									if (is_boss) 
									{
                                        // [ADD] TILE_BOSS_CHEST 생성
										g_dungeon_map[saved_map_y][saved_map_x] = TILE_BOSS_CHEST;
										if (saved_map_y - 1 >= 0 && g_dungeon_map[saved_map_y - 1][saved_map_x] != TILE_WALL) 
											g_dungeon_map[saved_map_y - 1][saved_map_x] = TILE_PORTAL;
										else 
											g_dungeon_map[saved_map_y][saved_map_x + 1] = TILE_PORTAL;
										game_state = 99;
									}
									else 
									{
								...
								if (current_monster != NULL && current_monster->hp > 0)
								{
									...
									if (is_boss)
									{
										if (current_monster->mp == 1)
										{
											...
											if (player.is_defending) { boss_dmg /= 4; sprintf(m1, "BLOCKED DEADLY BLOW!"); }
											else { sprintf(m1, "DEVASTATING STRIKE!"); }
											
											player.hp -= boss_dmg;
											sprintf(m_dmg_msg, "TOOK %d DAMAGE!", boss_dmg);
										...
										else if (GET_RAND(1, 100) <= 30)
										{
											current_monster->mp = 1;
											sprintf(m1, "THE BOSS IS GATHERING");
											render_typing_message(renderer, m1, "DARK ENERGY...", &player, current_monster, difficulty, is_boss, menu_index);
										}
										else
										{
											...
											player.hp -= m_dmg;
											sprintf(m1, "%s ATTACKED!", current_monster->name);
											sprintf(m_dmg_msg, "TOOK %d DAMAGE!", m_dmg);
											play_hit_effect(renderer, &player, current_monster, difficulty, is_boss, menu_index, 1);
											render_typing_message(renderer, m1, m_dmg_msg, &player, current_monster, difficulty, is_boss, menu_index);
										}
									}
									...
									{
										player.hp = 0;
										render_typing_message(renderer, "YOU DIED...", "", &player, current_monster, difficulty, is_boss, menu_index);
										game_state = 7;
									}
								}
								break;
				...
				else if (game_state == 4) // 스킬 창 로직
				{
					...
					else if (KEY == SDLK_RETURN)
					{
						...
						{ mp_cost = 15; skill_dmg = total_atk * 2; }
						else 
						{ mp_cost = 25; skill_dmg = total_atk * 3; }
				...
				else if (game_state == 5) // 아이템 창
				{
					...
					else if (KEY == SDLK_RETURN)
					{
						...
						if (menu_index == 0) 
						{
							if (hp_potions > 0)
							{
								hp_potions--; player.hp += 50;
								if (player.hp > player.max_hp) player.hp = player.max_hp;
								sprintf(m1, "USED HP POTION!"); sprintf(m2, "RECOVERED HP! LEFT: %d", hp_potions); item_used = 1;
							}
						...
						else if (menu_index == 1) 
						{
							if(mp_potions > 0)
							{
								mp_potions--; player.mp += 30;
								if (player.mp > player.max_mp) player.mp = player.max_mp;
								sprintf(m1, "USED MP POTION!"); sprintf(m2, "RECOVERED MP! LEFT: %d", mp_potions); item_used = 1;
							}
				...
				else if (game_state == 6)
				{
					...
					else if (KEY == SDLK_RETURN)
					{
						if(menu_index == 0) 
						{
							...
							{ if (!is_boss) current_monster->ignore_turns = 5; else current_monster->ignore_turns = 0; }
							current_monster = NULL; game_state = 99;       
						} 
						else { game_state = 3; }
						menu_index = 0;
					}		
		...
		if (game_state == 0)
		{
			...
			if (menu_index == 0) render_text(renderer, "START", 360, 310, 255, 255, 0);
			else render_text(renderer, "START", 360, 310, 255, 255, 255);

			if (menu_index == 1) render_text(renderer, "LOAD", 360, 355, 255, 255, 0);
			else render_text(renderer, "LOAD", 360, 355, 255, 255, 255);

			if (menu_index == 2) render_text(renderer, "EXIT", 360, 400, 255, 255, 0);
			else render_text(renderer, "EXIT", 360, 400, 255, 255, 255);
		...
		else if (game_state == 1)
		{
			...
			if (menu_index == 0) 
			{ render_text(renderer, ">", 50, 160, 255, 255, 0); render_text(renderer, "WARRIOR", 80, 160, 255, 255, 0); }
			...
			render_text(renderer, "ATK: 30", 80, 330, 200, 200, 200);

            // [UI 수정 위치: 마법사 정보 박스]
			draw_dw_window(renderer, 410, 140, 360, 280);
			if (menu_index == 1) 
			{ render_text(renderer, ">", 430, 160, 255, 255, 0); render_text(renderer, "MAGE", 460, 160, 255, 255, 0); }
			...
			for (int cy = 0; cy < 13; cy++)
			{
				for (int cx = 0; cx < 17; cx++)
				{
					...
					if (mx >= 0 && mx < MAP_WIDTH && my >= 0 && my < MAP_HEIGHT)
					{
						...
						if (t == TILE_WALL)
						{
							SDL_Rect s_r = { (difficulty == 0 ? 64 : (difficulty == 1 ? 112 : 0)), (difficulty == 0 ? 176 : (difficulty == 1 ? 16 : 0)), 16, 16 };
							SDL_RenderCopy(renderer, map_tex, &s_r, &d_r);
						}
						else
						{
							SDL_Rect s_r = { 16, (difficulty == 0 ? 128 : (difficulty == 1 ? 144 : 0)), 16, 16 };
							SDL_RenderCopy(renderer, map_tex, &s_r, &d_r);
		...
		else if (game_state >= 4 && game_state <= 6)
		{
			...
			if (game_state == 4)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					...
					FILL_RECT(opt); render_text(renderer, current_skills[i], 265, 270 + (i * 60), 0, 0, 0);
				}
			}
			else if (game_state == 5)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					...
					if (menu_index == i) SET_COLOR(255, 255, 0); else SET_COLOR(150, 150, 150);
					FILL_RECT(opt); render_text(renderer, item_menu[i], 265, 270 + (i * 60), 0, 0, 0);
				}
			}
			else if (game_state == 6)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					...
					if (menu_index == i) SET_COLOR(255, 255, 0); else SET_COLOR(150, 150, 150);
					FILL_RECT(opt); render_text(renderer, r_list[i], 350, 290 + (i * 50), 0, 0, 0);
				}
			}
		}
		else if (game_state == 7)
		{
			...
			if (player.hp <= 0) SET_COLOR(100, 0, 0); else SET_COLOR(0, 50, 100);
			FILL_RECT(full_bg);
        ...
        else if (game_state == STATE_SAVE_MENU || game_state == STATE_LOAD_MENU)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                char slot_info[128];
                if (g_save_slots[i].is_empty)
                {
                    sprintf(slot_info, "[SLOT %d] - EMPTY -", i + 1);
                    render_text(renderer, slot_info, 150, slot_y, 150, 150, 150);
                }
                else
                {
        ...
        else if (game_state == STATE_STATUS_MENU) 
        {
            ...
            sprintf(buf, "ATK  : %d (+%d)", player.atk, player.equip_weapon_atk); render_text(renderer, buf, 240, 420, 255, 255, 255);
        ...
        else if (game_state == STATE_EQUIP_MENU) 
        {
            ...
            render_text(renderer, "-- EQUIPMENT --", 310, 180, 255, 255, 0);
            
            char buf[64];
            sprintf(buf, "WEAPON: %s", player.weapon_name);
            render_text(renderer, buf, 240, 250, 255, 255, 255);
            sprintf(buf, "ARMOR : %s", player.armor_name);
            render_text(renderer, buf, 240, 300, 255, 255, 255);
            render_text(renderer, "PRESS ESC TO RETURN", 280, 400, 200, 200, 200);
        }
        else if (game_state == STATE_INVEN_MENU) 
        {
            ...
            render_text(renderer, "(MORE ITEMS EMPTY)", 280, 350, 150, 150, 150);
        ...
        else if (game_state == STATE_SHOP_UI)
        {
            // [UI 수정 위치: 상점창]
            draw_dw_window(renderer, 200, 150, 400, 300);
            render_text(renderer, "--- MERCHANT ---", 300, 180, 255, 255, 255);
            
            char price_buf[64];
            sprintf(price_buf, "BUY HP POTION (50G)"); render_text(renderer, price_buf, 260, 240, 255, 255, (menu_index == 0 ? 0 : 255));
            sprintf(price_buf, "BUY MP POTION (100G)"); render_text(renderer, price_buf, 260, 290, 255, 255, (menu_index == 1 ? 0 : 255));
            if (player.class_type == 0) sprintf(price_buf, "BUY IRON SWORD (300G)");
            else sprintf(price_buf, "BUY WOODEN WAND (300G)");
            render_text(renderer, price_buf, 260, 340, 255, 255, (menu_index == 2 ? 0 : 255));
            render_text(renderer, "LEAVE", 260, 390, 255, 255, (menu_index == 3 ? 0 : 255));
            
            int cursor_y = 240 + (menu_index * 50);
            render_text(renderer, ">", 230, cursor_y, 255, 255, 0);

            // [UI 수정 위치: 골드 표시 박스]
            draw_dw_window(renderer, 620, 150, 150, 100);
            render_text(renderer, "GOLD", 640, 170, 255, 255, 255);
            sprintf(price_buf, "%d G", player.gold); render_text(renderer, price_buf, 650, 210, 255, 255, 0);
        }
